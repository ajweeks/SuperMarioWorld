...
class PhysicsWeldJoint : public PhysicsJoint
{
public:
	//! PhysicsWeldJoint: the distance between two points on two bodies must be constant
	...
	PhysicsWeldJoint(PhysicsActor *actAPtr, DOUBLE2 anchorA, PhysicsActor *actBPtr, DOUBLE2 anchorB, double frequencyHz = 0, double dampingRatio = 0);
	virtual ~PhysicsWeldJoint();
	// C++11 make the class non-copyable
	PhysicsWeldJoint(const PhysicsWeldJoint&) = delete;
	PhysicsWeldJoint& operator=(const PhysicsWeldJoint&) = delete;
...
private:				 
	b2WeldJoint *m_WeldJointPtr = nullptr;
};
